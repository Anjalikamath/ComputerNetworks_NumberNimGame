...
'''
def chance(x):
	canvas.create_text(355,40,text= x+"'s Chance",font="Purisa",tags=ch, fill="black",command=None)	
	if(x=='BOB'):
		canvas.delete('AC')
'''
...
def on_click(event):
	#canvas.create_text(355,30,text="Alice's chance",font="Purisa",tags="Acp", fill="black",command=None)
		...
		try:
			...
			#when the player is playing and has not yet clicked turn done buttton
				...
				if piece_name=='DONE' and last_piece!='DONE' and last_piece!=None:
					...
					#canvas.delete("BP")
					#canvas.delete("BC")
					#canvas.create_text(355,50,text="Alice's chance",font="Purisa",tags="AC",fill="black",command=None)
...
def update_board(piece_names):
	...
	#for item in range(0, len(str(piece_names))): # multiple updates are required when the AI makes its moves
...
def end_game(winner):
	...
	for i in l:
		canvas.delete(i)
	send_moves(moves)
	#tcpCliSock.send(bytes(str("-1 "+winner), 'utf-8'))
